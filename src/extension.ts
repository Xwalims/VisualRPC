...
function getLangId(langId: string): string {
...
function detectLangFromFile(fileName: string): string | null {
  ...
  if (lower.endsWith(".cmake")) return "cmake";
  if (lower.endsWith(".toml")) return "toml";
  if (lower.endsWith(".yaml") || lower.endsWith(".yml")) return "yaml";
  return null;
}

function getLangName(langId: string, fileName?: string): string {
  let id = getLangId(langId);
  if (id === "plaintext" && fileName) {
    const detected = detectLangFromFile(fileName);
    if (detected) id = detected;
  }
...
function getLangAsset(langId: string, fileName?: string): string {
  let id = getLangId(langId);
  if (id === "plaintext" && fileName) {
    const detected = detectLangFromFile(fileName);
    if (detected) id = detected;
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("visualrpc.enable", () => rpc.enable())
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("visualrpc.disable", () => rpc.disable())
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("visualrpc.reconnect", () => rpc.reconnect())
  );
